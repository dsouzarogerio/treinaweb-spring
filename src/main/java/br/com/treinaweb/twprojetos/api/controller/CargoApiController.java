...
import br.com.treinaweb.twprojetos.api.docs.CargoApiControllerDoc;
...
public class CargoApiController implements CargoApiControllerDoc {
	...
	//Especificação para montar um recurso de paginação
	@Autowired
	...
	public CollectionModel<EntityModel<Cargo>> buscarTodos(Pageable paginacao){
		...
		//collectionModel sendo usado neste caso para retornar uma coleção de recursos (buscarTodos)
		return pagedResourcesAssembler.toModel(cargos, cargoAssembler);
	}

	@GetMapping("/{id}")
	public EntityModel<Cargo> buscarPorId(@PathVariable("id") Long id) {
