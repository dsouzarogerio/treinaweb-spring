...
public class ClienteAssembler implements SimpleRepresentationModelAssembler<Cliente>{
	...
	public void addLinks(EntityModel<Cliente> resource) {
		...
		Link selfLink = linkTo(methodOn(ClienteApiController.class).buscarPorId(id))
				.withSelfRel()
				.withType("GET");
		
		Link projetosLink = linkTo(methodOn(ClienteApiController.class).buscarPorProjetos(id))
				.withRel("projetos")
				.withType("GET");
	...
	public void addLinks(CollectionModel<EntityModel<Cliente>> resources) {
		Link selfLink = linkTo(methodOn(ClienteApiController.class).buscarTodos(null))
				.withSelfRel()
				.withType("GET");
